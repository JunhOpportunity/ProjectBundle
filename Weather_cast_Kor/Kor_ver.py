...
# 맨 위 오늘 날씨 프레임
today_frame = LabelFrame(root, text="오늘 날씨", width=350)
today_frame.pack(pady=10)

# 현재 온도
now_frame = LabelFrame(today_frame, text="실시간")
...
temperture_now.pack(padx=5, pady=10)
...
# 온도
today_etc_frame = LabelFrame(today_frame, text="그 외")
today_etc_frame.pack(side="right")
max_min_tem_label = Label(today_etc_frame, text="기온 최저 : {}ºC | 최고 : {}ºC".format(today_min_tem, today_max_tem))
# 구름
cloud_label = Label(today_etc_frame, text="구름 오전 : {} | 오후 : {}".format(today_am_cloud, today_pm_cloud))
...
moon_frame = LabelFrame(root, text="달 관측이 가능 할까요?")
moon_frame.pack(pady=10)
moon_answer = Label(moon_frame, text="답변 : {}  (◎/△/Ｘ)".format(moon_answer_icon))
moon_answer.pack(padx=76)

# 날씨 요약
summery_frame = LabelFrame(root, text="요약")
summery_frame.pack(pady=10)
# 오늘
summery_today_w_frame = LabelFrame(summery_frame, text="오늘")
summery_today_w_frame.pack(side="left")
s_t_am_c = Label(summery_today_w_frame, text="오전 강수확률 : {}".format(today_am_cloud))
s_t_pm_c = Label(summery_today_w_frame, text="오후 강수확률 : {}".format(today_pm_cloud))
...
summery_tomorrow_w_frame = LabelFrame(summery_frame, text="내일")
summery_tomorrow_w_frame.pack(side="left")
s_tom_am_c = Label(summery_tomorrow_w_frame, text="오전 강수확률 : {}".format(tomorrow_am_cloud))
s_tom_pm_c = Label(summery_tomorrow_w_frame, text="오후 강수확률 : {}".format(tomorrow_pm_cloud))
...
summery_aft_tomorrow_w_frame = LabelFrame(summery_frame, text="내일 모레")
summery_aft_tomorrow_w_frame.pack(side="left")
s_a_t_am_c = Label(summery_aft_tomorrow_w_frame, text="오전 강수확률 : {}".format(aft_tomorrow_am_cloud))
s_a_t_pm_c = Label(summery_aft_tomorrow_w_frame, text="오후 강수확률 : {}".format(aft_tomorrow_pm_cloud))
s_a_t_t = Label(summery_aft_tomorrow_w_frame, text="{}ºC/{}ºC".format(aft_tom_min_tem, aft_tom_max_tem))
s_a_t_am_c.pack()
s_a_t_pm_c.pack()
s_a_t_t.pack()

refresh_button = Button(root, text="새로고침", command=refresh, bg="cornflowerblue")
refresh_button.pack()
